...
it('GeoJsonDataParser is defined', function () {
    expect(GeoJsonDataParser).toBeDefined();
});
describe('GeoJsonDataParser implements DataParser', function () {
    it('readData is defined', function () {
        var gjParser = new GeoJsonDataParser();
        expect(gjParser.readData).toBeDefined();
    });
});
describe('readData implementation', function () {
    var geojson = {
    ...
    it('works as expected and returns the correct data', function () {
        var gjParser = new GeoJsonDataParser();
        var output = gjParser.readData(geojson);
    ...
    it('exits when an invalid geojson is given as input', function () {
        var invalidGeojson = {
            type: 'FeatureCollection',
            foo: 'bar',
            kalle: 'Berga'
    ...
    it('exits when an invalid data is given as input', function () {
        var invalidData = '<?xml version="1.0" encoding="UTF-8"?><foo>123</foo>';
        var gjParser = new GeoJsonDataParser();
        var output;
        try {
...
//# sourceMappingURL=GeoJsonDataParser.spec.js.map